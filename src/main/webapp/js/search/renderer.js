/*global Ext, vardb, window */
(Ext.ux.vardb.Renderer=function(){	return {

	renderAccession:function(value, p, r)
	{
		return String.format('<a href="'+vardb.webapp+'/sequences/{0}.html" target="_blank">{1}</a>', r.data.identifier, value);
	},
	
	renderAccessionPopup:function(value, p, r)
	{
		return String.format('<a href="javascript:void(0)" onclick="vardb.sequencePopup(\'{0}\')">{1}{2}</a>',
	...
	{
		return String.format('<a href="'+vardb.webapp+'/pathogens/{0}.html">{1}</a>', r.data.pathogen_identifier, value);
	},
	...
	{
		return String.format('<a href="'+vardb.webapp+'/families/{0}.html">{1}</a>', r.data.family_identifier, value);
	},
	...
	{
		return String.format('<a href="'+vardb.webapp+'/subgroups/{0}.html">{1}</a>', r.data.subgroup_identifier, value);
	},
	...
	{
		return String.format('<a href="'+vardb.webapp+'/genomes/{0}.html">{1}</a>', r.data.genome_identifier, value);
	},
	...
	{
		return String.format('<a href="'+vardb.webapp+'/browser.html?genome={0}">{1}</a>', r.data.genome_identifier, value);
	},
	...
	{
		return String.format('<a href="'+vardb.webapp+'/diseases/{0}.html">{1}</a>', r.data.disease_identifier, value);
	},
	...
	{
		return Ext.ux.vardb.Renderer.renderRefPopup(value,p,r);
	},
	...
	renderTags:function(value, p, r, list_id)
	{
		...
		for (index=0;index<tags_name.length;index++)
		{
			...
			if (description!=='')
				{title+=': '+description;}
			str='<a href="javascript:void(0)" onclick="Ext.ux.vardb.tags.Services.editTag(\''+tag_id+'\')"'+
	...
	renderGroup:function(value, p, r)
	{
		...
		var str='<a href="javascript:void(0)" onclick="Ext.ux.vardb.Vardb.editGroup(\''+r.data.group_id+'\')"'+
	...
	renderAlignment:function(value, p, record)
	{
		return String.format('<a href="'+vardb.webapp+'/alignments/{1}.html">{0}</a>',
